...
func TestRunTraceM3Goldens(t *testing.T) {
	tests := []struct {
		...
		{name: "method references", fixture: "constructors-methodrefs", target: "app.Workflow.references", golden: "expected-references.mmd"},
		{name: "nested target", fixture: "constructors-methodrefs", target: "refs.References.Nested.run", golden: "expected-nested.mmd"},
		{name: "dispatch", fixture: "dispatch", target: "app.Workflow.start", golden: "expected.mmd"},
